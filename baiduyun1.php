...
$cookie_tmp = get_cookie($url, $user_agent, $ip);
$vcode_str  = get_code($cookie_tmp, $ip);
for ($i = $start, $j = 1; $i < $end; $i ++, $j ++)
{
	...
	$str = get_str($i);
	ll($h, $i.'---'.$str, FALSE);
	$re = get_pan($url, $cookie_tmp, $user_agent, $str, $vcode_str, $h, $ip);
	if ($re)
	{
		ll($h, $i.'::::::ok::::::::'.$str);
		break;
...
function get_cookie($url, $user_agent, $ip)
{
	// 根据地址获取百度网盘的真正地址
	$ch = curl_init($url);
...
function get_pan($url, $cookie_tmp, $user_agent, $pwd = '', $vcode_str = '', $h, $ip)
{
