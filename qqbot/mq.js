  ...
  J.$package("mq.i18n", function (J) {
    ...
    var faceTextArr = [
      [
        ...
        "激动", "街舞", "献吻", "左太极", "右太极", "双喜", "鞭炮", "灯笼", "发财", "K歌", "购物", "邮件", "帅", "喝彩", "强"/* "祈祷" */, "爆筋", "棒棒糖", "喝奶", "下面", "香蕉",
...
//联系人presenter类
  J.$package("mq.presenter.buddylist", function (J) {
    ...
    var handlers = {
      ...
      onDispatchMessage: function (message) {
        pageContext.sessionView.onDispatchMessage(message);
      },
    ...
    this.bindHandlers = function () {
      ...
      $E.on(chatModel, "messageDispatch", handlers.onDispatchMessage);
...
//聊天model类
  J.$package("mq.model.chat", function (J) {
    ...
    // 设置同步消息
    this.addSyncMessage = function (uin, msg) {
      if (!self_msg[uin]) self_msg[uin] = [];
      self_msg[uin].push(msg);
      if (!uin2msgArr[uin]) uin2msgArr[uin] = [];
      uin2msgArr[uin].push(msg);

      $E.fire(packageContext, "messageDispatch", msg);
    };
        ...
        //好友，陌生人等消息
          ...
          case "message":
            ...
            if (from_user) {
              ...
              // 同步自己发送的消息
              // return ;
              if (packageContext.m_model.getSelfUin() === from_uin) {
              }
              {
                var to_user = packageContext.m_model.getFriendByUin(msg.to_uin);
                from_user = packageContext.m_model.getSelfInfo();
                senderMsgObj = {
                  content: msg.content,
                  from_uin: from_uin,
                ...
                // console.log(121, senderMsgObj);
                packageContext.addSyncMessage(msg.to_uin, senderMsgObj);
                // packageContext.addMessage(msg.to_uin, senderMsgObj);
              }
            }
            break;
          case "group_message":
            ...
            if (send_user === undefined) {
              if (msg.send_uin === packageContext.m_model.getSelfUin()) {
                send_user = packageContext.m_model.getSelfInfo();
              } else {
...
define('mq.presenter.chat', [
  ...
  J.$package("mq.presenter.chat", function (J) {
    ...
    var handlers = {
      ...
      onMessageSended: function (msg) {
        var user = msg.to_user;
        if (currentChatType === 'friend' && currentChatUin === user.uin) {
          packageContext.appendMessage([msg]);
        }
      },
    ...
    var bindHandlers = function () {
      ...
      $E.on(m, "messageDispatch", handlers.onMessageSended);
      ...
      $E.on(mq.model.chat, 'sendMessage', handlers.onSendMessage);
...
define('mq.view.session', [
  ...
  J.$package("mq.view.session", function (J) {
    ...
    this.onDispatchMessage = function (message) {
      this.updateMessageSend(message);
    }
    ...
    this.updateMessageSend = function (message) {
      var to = message.to_user;
      var key = to.type + '-' + to.account;
      var domId = 'recent-item-' + key;
      var dom = $D.id(domId);
      ...
      if (!dom) {
        ...
        this.memberList.append({type: 'list', list: [to]}, true);
        dom = $D.id(domId);
        if (!dom) {//这里如果还没有找到dom,可能是出错了
          return;
        }
  ...
  J.$package("mq.view.loginPanel", function (J) {
    ...
    var getLoginUrl = function () {
      var params = {
        ...
        enable_qlogin: 0,
        no_verifyimg: 1,
        s_url: location.protocol + '//web2.qq.com/proxy.html',
