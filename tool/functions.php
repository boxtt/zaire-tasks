...
// 逐行读取TXT文件   end_line 0：表示到最后，null表示默认为100
function get_content_by_line($txtfile, $start_line = 0, $end_line = NULL)
{
 ...
 * @param string $target_path   新图片要保存的地址，全路径，包括名字
...
function image_resize($source_path, $target_path, $target_width = 200, $target_height = 200, $fixed_orig = '')
{
	...
	imagecopyresampled($target_image, $source_image, 0, 0, 0, 0, $target_width, $target_height, $source_width,
		$source_height);
